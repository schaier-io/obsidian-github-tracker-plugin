...
export class GitHubTrackerSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .addDropdown((dropdown) => {
        dropdown
          ...
          .onChange(async (value) => {
            this.plugin.settings.syncNoticeMode = value as "minimal" | "normal" | "extensive" | "debug";
            await this.plugin.saveSettings();
          });
      });
    ...
    new Setting(containerEl)
      ...
      .addDropdown((dropdown) =>
        dropdown
          .addOption("disabled", "Disabled - No escaping (may allow malicious content)")
          .addOption("normal", "Normal - Basic escaping for plugins like templater and dataview")
          .addOption("strict", "Strict - Only alphanumeric characters and links will be allowed")
          .addOption("veryStrict", "Very strict - Only alphanumeric characters, and punctuation")
          .setValue(this.plugin.settings.escapeMode)
          .onChange(async (value) => {
            if (value === "disabled") {
              const modal = new Modal(this.app);
              modal.titleEl.setText("Security Warning");
              modal.contentEl.setText("Disabling body content escaping may allow malicious scripts to execute in your vault. Are you sure you want to continue?");
              ...
              continueButton.onclick = async () => {
                this.plugin.settings.escapeMode = value as "disabled" | "normal" | "strict" | "veryStrict";
                await this.plugin.saveSettings();
                modal.close();
              };
    ...
    const infoText = containerEl.createEl("p", {text:"CAUTION: especially if using Plugins that enable script execution. In disabled mode, no escaping will be done. "});
    infoText.addClass("github-tracker-info-text");
    infoText.addClass("github-tracker-warning-text");

    const infoText2 = containerEl.createEl("p", {text:"In normal mode '`', '{{', '}}', '<%' and '%>' will be escaped. (This has the side effect of not allowing code blocks to be rendered)"});
    infoText2.addClass("github-tracker-info-text");

    const infoText3 = containerEl.createEl("p", {text:"In strict mode only alphanumeric characters, '.,'()/[]{}*+-:\"' and whitespace will be allowed. This will remove any html like rendering and templating, but persist links"});
    infoText3.addClass("github-tracker-info-text");

    const infoText4 = containerEl.createEl("p", {text:"In very strict mode only alphanumeric characters, and '.,' or whitespace will be allowed. This will remove any html like rendering and templating."});
    ...
    infoLink.href="https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token";
          ...
          .onChange((value) => {
            ...
            repoElements.forEach((element) => {
              ...
              if (repoName.includes(searchTerm)) {
                (element as HTMLElement).classList.remove("github-tracker-hidden");
                visibleRepositories++;
              } else {
                (element as HTMLElement).classList.add("github-tracker-hidden");
              }
            });
            noRepositories.classList.toggle("github-tracker-hidden", visibleRepositories > 0);
          })
      );
    ...
    noRepositories.classList.toggle("github-tracker-hidden", this.plugin.settings.repositories.length > 0);
  ...
  private async renderGitHubRepositories(container: HTMLElement): Promise<void> {
    container.empty();
    ...
    try {
      ...
      for (const repo of repos) {
        ...
        const isTracked = this.plugin.settings.repositories.some(r => r.repository === repoName);
      ...
      container.empty();
      container.createEl("p", { text: `Error loading repositories: ${(error as Error).message}` });
    }
  }
  
  private async addRepository(repoName: string): Promise<void> {
    // Check if repository already exists
    if (this.plugin.settings.repositories.some(r => r.repository === repoName)) {
  ...
  private renderRepositoriesList(container: HTMLElement): void {
    const reposContainer = container.createDiv("github-tracker-repos-container");
    
    // Render each repository
    for (const repo of this.plugin.settings.repositories) {
      const repoContainer = reposContainer.createDiv("github-tracker-repo-settings");
      repoContainer.setAttribute("data-repo-name", repo.repository);
      
      // Create repository header
      const repoHeader = repoContainer.createDiv("github-tracker-repo-header");
      // Replace h3 with setting + setHeading
      new Setting(repoHeader).setName("Repository: "+repo.repository).setHeading();
      
      // Create repository actions
      const repoActions = repoHeader.createDiv("github-tracker-repo-actions");
      ...
      const issuesContainer = repoContainer.createDiv("github-tracker-settings-section");
      const pullRequestsContainer = repoContainer.createDiv("github-tracker-settings-section");
  ...
  private async showDeleteRepositoryModal(repo: RepositoryTracking): Promise<void> {
    const modal = new Modal(this.app);
    modal.titleEl.setText("Delete Repository");
    modal.contentEl.setText(`Are you sure you want to delete ${repo.repository}? This will remove all tracking settings for this repository.`);
  ...
  private renderIssueSettings(container: HTMLElement, repo: RepositoryTracking): void {
    ...
    new Setting(container)
      ...
      .addToggle((toggle) =>
        toggle
          .setValue(repo.trackIssues)
          .onChange(async (value) => {
            repo.trackIssues = value;
            issuesSettingsContainer.classList.toggle("github-tracker-settings-hidden", !value);
            await this.plugin.saveSettings();
          })
      );

    // Issues additional settings container
    const issuesSettingsContainer = container.createDiv("github-tracker-settings-group");
    issuesSettingsContainer.classList.toggle("github-tracker-settings-hidden", !repo.trackIssues);
    ...
    new Setting(issuesSettingsContainer)
      ...
      .addDropdown((dropdown) =>
        dropdown
          ...
          .onChange(async (value) => {
            repo.issueUpdateMode = value as "none" | "update" | "append";
            await this.plugin.saveSettings();
          })
      );
      ...
      .addToggle((toggle) =>
        toggle
          ...
          .onChange(async (value) => {
            ...
            assigneeMatchSetting.settingEl.classList.toggle("github-tracker-settings-hidden", !value);
            await this.plugin.saveSettings();
          })
      );

    // Issues assignee match setting
    const assigneeMatchSetting = new Setting(issuesSettingsContainer)
    ...
    assigneeMatchSetting.settingEl.classList.toggle("github-tracker-settings-hidden", !repo.requireAssignee);
      ...
      .addToggle((toggle) =>
        toggle
          ...
          .onChange(async (value) => {
            ...
            openedByIssueMatchSetting.settingEl.classList.toggle("github-tracker-settings-hidden", !value);
            await this.plugin.saveSettings();
          })
      );

    // Issues opened by match setting
    const openedByIssueMatchSetting = new Setting(issuesSettingsContainer)
      .setName("Opened by")
      .setDesc(
    ...
    openedByIssueMatchSetting.settingEl.classList.toggle("github-tracker-settings-hidden", !repo.requireOpenedByIssue);
  }
  
  private renderPullRequestSettings(container: HTMLElement, repo: RepositoryTracking): void {
      ...
      .addToggle((toggle) =>
        toggle
          ...
          .onChange(async (value) => {
            ...
            pullRequestsSettingsContainer.classList.toggle("github-tracker-settings-hidden", !value);
            await this.plugin.saveSettings();
          })
      );

    // Pull Requests additional settings container
    const pullRequestsSettingsContainer = container.createDiv("github-tracker-settings-group");
    pullRequestsSettingsContainer.classList.toggle("github-tracker-settings-hidden", !repo.trackPullRequest);
    ...
    new Setting(pullRequestsSettingsContainer)
      ...
      .addDropdown((dropdown) =>
        dropdown
          .addOption("none", "None - Don't update existing pull requests")
          .addOption("update", "Update - Overwrite existing content")
          .addOption("append", "Append - Add new content at the end")
          .setValue(repo.pullRequestUpdateMode)
          .onChange(async (value) => {
            repo.pullRequestUpdateMode = value as "none" | "update" | "append";
            await this.plugin.saveSettings();
          })
      );
      ...
      .addToggle((toggle) =>
        toggle
          ...
          .onChange(async (value) => {
            ...
            reviewerMatchSetting.settingEl.classList.toggle("github-tracker-settings-hidden", !value);
            await this.plugin.saveSettings();
          })
      );

    // Pull Requests reviewer match setting
    const reviewerMatchSetting = new Setting(
    ...
    reviewerMatchSetting.settingEl.classList.toggle("github-tracker-settings-hidden", !repo.requireReviewer);

    // Pull Requests require assignee toggle
    new Setting(pullRequestsSettingsContainer)
      .setName("Require assignee")
      .setDesc(
        "Only track pull requests that have at least one assignee"
      )
      .addToggle((toggle) =>
        toggle
          ...
          .onChange(async (value) => {
            ...
            pullRequestAssigneeMatchSetting.settingEl.classList.toggle("github-tracker-settings-hidden", !value);
            await this.plugin.saveSettings();
          })
      );

    // Pull Requests assignee match setting
    const pullRequestAssigneeMatchSetting = new Setting(
      pullRequestsSettingsContainer
    )
      .setName("Assignee match")
      .setDesc(
    ...
    // Set initial visibility of assignee match setting
    pullRequestAssigneeMatchSetting.settingEl.classList.toggle("github-tracker-settings-hidden", !repo.requirePullRequestAssignee);
    
    // Pull Requests require opened by toggle
    new Setting(pullRequestsSettingsContainer)
      .setName("Require opened by")
      .setDesc("Only track pull requests that were opened by a specific user")
      .addToggle((toggle) =>
        toggle
          .setValue(repo.requireOpenedByPR)
          .onChange(async (value) => {
            repo.requireOpenedByPR = value;
            openedByPRMatchSetting.settingEl.classList.toggle("github-tracker-settings-hidden", !value);
            await this.plugin.saveSettings();
          })
      );

    // Pull Requests opened by match setting
    const openedByPRMatchSetting = new Setting(pullRequestsSettingsContainer)
      .setName("Opened by")
      .setDesc(
    ...
    // Set initial visibility of opened by match setting
    openedByPRMatchSetting.settingEl.classList.toggle("github-tracker-settings-hidden", !repo.requireOpenedByPR);
  }
} 