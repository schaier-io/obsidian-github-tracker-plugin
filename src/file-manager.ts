	...
	public filterIssues(repo: RepositoryTracking, issues: any[]): any[] {
		return issues.filter((issue) => {
			...
			if (repo.requireAssignee) {
				...
				if (
					...
					assigneeNames.some(
						(name: string) =>
							name.toLowerCase() ===
							repo.assigneeMatch.toLowerCase()
					)
				) {
					return true;
				}
			...
			if (repo.requireOpenedByIssue) {
				if (
					repo.openedByIssueMatch &&
					issue.user.login &&
					issue.user &&
					issue.user.login.toLowerCase() ==
						repo.openedByIssueMatch.toLowerCase()
				) {
					return true;
				}
			}
			...
			if (repo.requireIssueLabel) {
				const labelNames =
					issue.labels?.map((l: { name: string }) => l.name) || [];
				if (
					repo.issueLabelMatch &&
					repo.issueLabelMatch.length > 0 &&
					labelNames.length > 0 &&
					repo.issueLabelMatch.some((label: string) =>
						labelNames.some(
							(name: string) =>
	...
	public filterPullRequests(
		...
		return pullRequests.filter((pr) => {
			...
			if (repo.requireReviewer) {
				...
				if (
					...
					reviewerNames.some(
						(name: string) =>
							name.toLowerCase() ===
							repo.reviewerMatch.toLowerCase()
					)
				) {
					return true;
				}
			...
			if (repo.requirePullRequestAssignee) {
				...
				if (
					repo.pullRequestAssigneeMatch &&
					assigneeNames.length > 0 &&
					assigneeNames.some(
						(name: string) =>
							name.toLowerCase() ===
							repo.pullRequestAssigneeMatch.toLowerCase()
					)
				) {
					return true;
				}
			...
			if (repo.requireOpenedByPR) {
				if (
					repo.openedByPRMatch &&
					pr.user.login &&
					pr.user &&
					pr.user.login.toLowerCase() ==
						repo.openedByPRMatch.toLowerCase()
				) {
					return true;
				}
			}
			...
			// Check label requirement
			if (repo.requirePullRequestLabel) {
				const labelNames =
					pr.labels?.map((l: { name: string }) => l.name) || [];
				if (
					repo.pullRequestLabelMatch &&
					repo.pullRequestLabelMatch.length > 0 &&
					labelNames.length > 0 &&
					repo.pullRequestLabelMatch.some((label: string) =>
						labelNames.some(
							(name: string) =>
								name.toLowerCase() === label.toLowerCase()
						)
